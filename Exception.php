...
class Exception
{
    ...
    {
        \Agl\Core\Request\Request::setHttpHeader(\Agl\Core\Request\Request::HEADER_500);
    ...
    {
        $logId = \Agl\Core\Debug\Debug::log("Error '$pErrstr' in '$pErrfile' on line $pErrline");
...
<strong>Line</strong> ' . $pErrline . '
    ...
    {
        $logId = \Agl\Core\Debug\Debug::log("Exception '" . $pException->getMessage() . "' in '" . $pException->getFile() . "' on line " . $pException->getLine());
...
' . $pException->getTraceAsString() . '
    ...
    public static function shutdownHandler()
    {
        ...
        if ($error) {
            $logId = \Agl\Core\Debug\Debug::log("Error '" . $error['message'] . "' in '" . $error['file'] . "' on line " . $error['line']);
...
set_exception_handler(array('\Agl\Exception', 'exceptionHandler'));
register_shutdown_function(array('\Agl\Exception', 'shutdownHandler'));
