        ...
        implements \Agl\Core\Db\Query\Drop\DropInterface
{
    ...
    private function _checkCommitResult(array $result)
    {
        if (! $result['ok'] and $result['errmsg']) {
            throw new \Agl\Exception("Delete failed to MongoDB collection '" . $this->_prefix . $this->_collection->getDbContainer() . "' with error message '" . $result['errmsg'] . "'");
        } else if (! $result['ok']) {
            throw new \Agl\Exception("Delete failed to MongoDB collection '" . $this->_prefix . $this->_collection->getDbContainer() . "' with no error message");
        }
    }
    ...
    public function commit()
    {
        try {
            ...
            throw new \Agl\Exception("Insert (safe mode) failed to MongoDB collection '" . $this->_prefix . $this->_collection->getDbContainer() . "' with message '" . $e->getMessage() . "'");
        } catch (\Exception $e) {
