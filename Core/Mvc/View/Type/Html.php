		...
		implements \Agl\Core\Mvc\View\ViewInterface
{
	...
	public function loadCss()
	{
		...
		$templateCss = \Agl::app()->getConfig('@layout/modules/' . $module . '/' . $view . '/template/css');
		if ($templateCss === NULL) {
			$templateCss = \Agl::app()->getConfig('@layout/template/css');
		}
		$this->_loadCssFromArray($templateCss);

		$viewCss = \Agl::app()->getConfig('@layout/modules/' . $module . '/' . $view . '/css');
		$this->_loadCssFromArray($viewCss);

		foreach ($this->_blocks as $block) {
			$blocksCss = \Agl::app()->getConfig('@layout/blocks/' . $block['group'] . '/' . $block['block'] . '/css');
			$this->_loadCssFromArray($blocksCss);
		}
	...
	public function loadJs()
	{
		...
		$templateJs = \Agl::app()->getConfig('@layout/modules/' . $module . '/' . $view . '/template/js');
		if ($templateJs === NULL) {
			$templateJs = \Agl::app()->getConfig('@layout/template/js');
		}
		$this->_loadJsFromArray($templateJs);

		$viewJs = \Agl::app()->getConfig('@layout/modules/' . $module . '/' . $view . '/js');
		$this->_loadJsFromArray($viewJs);

		foreach ($this->_blocks as $block) {
			$blocksJs = \Agl::app()->getConfig('@layout/blocks/' . $block['group'] . '/' . $block['block'] . '/js');
			$this->_loadJsFromArray($blocksJs);
		}
