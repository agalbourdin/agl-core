...
class Json
    implements \Agl\Core\Config\ConfigInterface
{
    /**
    ...
    public static function getCacheSingleton()
    {
        if (self::$_cacheInstance === NULL) {
            self::$_cacheInstance = new \Agl\Core\Cache\File\Format\Arr(self::CONFIG_CACHE_ID);
        }
    ...
    public function __construct()
    {
        ...
        if ($this->_cacheEnabled) {
            $this->_apcEnabled   = \Agl\Core\Cache\Apc\Apc::isApcEnabled();
            if ($this->_apcEnabled) {
                $cache = \Agl\Core\Cache\Apc\Apc::get(self::CONFIG_CACHE_ID);
                if (is_array($cache)) {
                    $this->_cache = $cache;
                }
            } else {
    ...
    {
        $path = str_replace('@layout', '@module[' . \Agl::AGL_CORE_DIR . '/' . \Agl\Core\Mvc\View\ViewInterface::CONFIG_FILE . ']', $pPath);
    ...
    public function getConfig($pPath, $pForceGlobalArray = false)
    {
        if ($this->_cacheEnabled) {
            if ($this->_apcEnabled) {
                ...
                if ($value !== \Agl\Core\Cache\File\FileInterface::AGL_CACHE_TAG_NOT_FOUND) {
                    return $value;
                }
            }
        }
        ...
        if ($this->_cacheEnabled) {
            if ($this->_apcEnabled) {
                \Agl\Core\Cache\Apc\Apc::set(self::CONFIG_CACHE_ID, $this->_cache);
            } else {
                self::$_cacheInstance
