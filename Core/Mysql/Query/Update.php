        ...
        try {
            ...
            if (! empty($toUpdate)) {
                $query = "
                    ...
                    WHERE
                        `" . $this->_item->getDbContainer() . \Agl\Core\Db\Item\ItemInterface::PREFIX_SEPARATOR . \Agl\Core\Db\Item\ItemInterface::IDFIELD . "` = ?";

                if (! array_key_exists(\Agl\Core\Db\Item\ItemInterface::IDFIELD, $toUpdate)) {
                    $toUpdate[\Agl\Core\Db\Item\ItemInterface::IDFIELD] = $this->_item->getId()->getOrig();
                }
